...
# Criar o experimento
mlflow.create_experiment("exp_projeto_ciclo_2_V3")

# Definir o experimento atual
mlflow.set_experiment("exp_projeto_ciclo_2_V3")
...
for i in range(folds):
    ...
    with mlflow.start_run(run_name=f"Fold_{i + 1}"):
        ...
        knn = BaggingClassifier(estimator=KNeighborsClassifier(n_neighbors=15, weights='uniform'), n_estimators=10, max_samples=0.6, max_features=0.6)
        svc = AdaBoostClassifier(estimator=SVC(kernel='poly', probability=True, C=0.5, gamma='auto'), n_estimators=10,learning_rate=0.8, algorithm='SAMME')
        rf = BaggingClassifier(estimator=RandomForestClassifier(n_estimators=30, max_depth=15, min_samples_split=5), n_estimators=10, max_samples=0.75, max_features=0.75)
