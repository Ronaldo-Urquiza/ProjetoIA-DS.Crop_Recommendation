import os
from matplotlib import pyplot as plt # Mostrar gráficos
...
# Cria nossa árvore com nome "model"
model = tree.DecisionTreeClassifier(criterion="entropy") #ela decide os atributos significativos (melhores) conforme a entropia deles
model = model.fit(X_train, y_train) #traduzindo:
                                    #nossa árvore chamada "model" vai se modelar com as variáveis (X_train, y_train) que são os 80% do dataframe reservados para treinamento
...
# Gera a visualização da árvore de decisão e salva como arquivo SVG
plt.figure(figsize=(20,10))  # Define o tamanho da figura
tree.plot_tree(model, filled=True) # Gera a figura
plt.savefig('1_Tree.svg', format='svg')  # Salva a figura da árvore como um arquivo SVG para não perder a qualidade dado o tamanho da árvore